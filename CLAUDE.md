...
- `lina-image.js` + `image.html`: photo to halftone / stems / bitmap. Squares only in halftone, in half-cell sizes; a checkerboard bridge band around half tone (not with staggered rows) and tone-gated merging in the darks. `lina-sample.js` is the embedded sample portrait.
