...
- One declared exception outside the logo: the landing page's animated dither field (the banner under the header in `index.html`) is decoration, not drawn by the rule. Its dots vary in size continuously; their corners keep a fixed radius of ¼ of the dot pitch, so dots smaller than half the pitch become circles. It stays confined to that banner; no tool or other page may reuse it.
